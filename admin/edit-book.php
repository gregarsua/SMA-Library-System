...
$sql="UPDATE  tblbooks set BookName=:bookname,CatId=:category,
    AuthorId=:author,bid=:isbn,BookPrice=:price,Stock=:stock where bid=:bookid";
...
<?php 
                ...
                $sql = "SELECT tblbooks.BookName,tblcategory.CategoryName,
                        tblcategory.id as cid,tblauthors.AuthorName,tblauthors.id 
                        as athrid,tblbooks.BookPrice,tblbooks.Stock,tblbooks.bid 
                        as bookid from  tblbooks join tblcategory on tblcategory.id=tblbooks.CatId 
                        join tblauthors on tblauthors.id=tblbooks.AuthorId where tblbooks.bid=:bookid";
                $query = $dbh -> prepare($sql);
                $query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
                $query->execute();
...
<input class="form-control" type="text" name="isbn" value="<?php echo htmlentities($result->bookid);?>"  required="required" />
